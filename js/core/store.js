...
export function seedStarter() {
  ...
  store.commit(s => {
    ...
    s.metrics = [{ id: uid('met'), date: t, water: 3, steps: 4200, sleep: 7.5, weight: null }];
    s.activity = [{ id: uid('act'), icon: 'sparkles', text: 'GabikOS installed — welcome aboard', view: 'dashboard', ts: Date.now() }];
  }, { action: 'seed' });
}
